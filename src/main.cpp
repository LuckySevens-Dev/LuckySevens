...
#if defined(NDEBUG)
# error "LuckySevens cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc6ff9be5b895f43f22c1897dcb32d53b5bdb6f963e94952aef9b6e8e73ea2493");
...
const string strMessageMagic = "LuckySevens Signed Message:\n";
...
{
    // LuckySevens: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 7 * 60; // 7 minutes
static const int64 nTargetSpacing = 77; // 77 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LuckySevens: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LuckySevens: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc6ff9be5b895f43f22c1897dcb32d53b5bdb6f963e94952aef9b6e8e73ea2493");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2015/07/27 Time for LuckySevens";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("154475821eb689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1438046458;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 308601;

        if (fTestNet)
        {
            block.nTime    = 1438046438;
            block.nNonce   = 308601;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc4162f3d49e8413c67c11f6da5cdead7d630df443573c5f4482ac79ae9e78ef5"));
...
unsigned char pchMessageStart[4] = { 0xf6, 0xe5, 0xc4, 0xce }; // LuckySevens: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LuckySevensMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LuckySevensMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LuckySevensMiner(CWallet *pwallet)
{
    printf("LuckySevensMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("luckysevens-miner");
    ...
    try { loop {
        ...
        printf("Running LuckySevensMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LuckySevensMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LuckySevensMiner, pwallet));
}
