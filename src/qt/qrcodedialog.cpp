...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("L7S")
    updateDisplayUnit();
}
...
{
    QString ret = QString("luckysevens:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non L7S unit input in lnReqAmount, we generate the URI with L7S as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::L7S, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
